    ...
    getCDN: async () => {
        ...
        try {
            ...
            const apiPromise = savetube.request(savetube.api.cdn, {}, 'get', CONFIG.FAST_TIMEOUT)
                .then(r => r.status ? r.data.cdn : Promise.reject('API CDN failed'));
            promises.push(apiPromise);

            const firstAvailableServer = await Promise.any(promises);

            console.log('\x1b[32m%s\x1b[0m', '[CDN]', `Usando servidor: ${firstAvailableServer}`);
            cache.set(cacheKey, { data: firstAvailableServer, timestamp: Date.now() });
            return { status: true, data: firstAvailableServer };
